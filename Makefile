...
SRC = Migrator.cpp Free_Space_Meter.cpp Utilization_Meter.cpp Workload_Definitions.cpp Garbage_Collector.cpp Scheduling_Strategies.cpp events_queue.cpp wear_leveling_strategy.cpp grace_hash_join.cpp exp_sequential.cpp page_ftl.cpp address.cpp block.cpp config.cpp die.cpp event.cpp package.cpp page.cpp plane.cpp ram.cpp ssd.cpp scheduler.cpp bm_shortest_queue.cpp page_hotness_measurer.cpp bm_wearwolf.cpp bm_wearwolf_locality.cpp  bm_hot_cold_seperation.cpp bm_parent.cpp visual_tracer.cpp state_visualiser.cpp statistics_gatherer.cpp operating_system.cpp thread_implementations.cpp class sequential_pattern_detector.cpp mtrand.cpp external_sort.cpp bm_round_robin.cpp File_Manager.cpp random_order_iterator.cpp experiment_runner.cpp flexible_reader.cpp
OBJ = Migrator.o Free_Space_Meter.o Utilization_Meter.o Workload_Definitions.o Garbage_Collector.o Scheduling_Strategies.o events_queue.o wear_leveling_strategy.o grace_hash_join.o page_ftl.o address.o block.o config.o die.o event.o package.o page.o plane.o ram.o ssd.o scheduler.o bm_shortest_queue.o page_hotness_measurer.o bm_wearwolf.o bm_wearwolf_locality.o bm_hot_cold_seperation.o bm_parent.o visual_tracer.o state_visualiser.o statistics_gatherer.o operating_system.o thread_implementations.o sequential_pattern_detector.o mtrand.o external_sort.o bm_round_robin.o File_Manager.o random_order_iterator.o experiment_runner.o flexible_reader.o
LOG = log
PERMS = 660
EPERMS = 770
...
all: overprov #scheduling sequential_tuning sequential greediness copybacks

overprov: $(HDR) $(OBJ)
	$(CXX) $(CXXFLAGS) -o Experiments/overprov Experiments/over_prov.cpp $(OBJ)
	-chmod $(PERMS) $(OBJ) 
	-chmod $(EPERMS) Experiments/overprov

deadlines: $(HDR) $(OBJ)
	$(CXX) $(CXXFLAGS) -o Experiments/deadlines Experiments/deadlines.cpp $(OBJ)
	-chmod $(PERMS) $(OBJ) 
