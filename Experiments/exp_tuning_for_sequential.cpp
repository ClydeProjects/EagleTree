...
int main()
{
	...
	exp.push_back( Experiment::overprovisioning_experiment(greedy_eq_prio,	space_min, space_max, space_inc, exp_folder + 		"greedy_eq_prio/", "greedy eq prio", IO_limit) );
	exp.push_back( Experiment::overprovisioning_experiment(greedy_gc_prio,	space_min, space_max, space_inc, exp_folder + 		"greedy_gc_prio/", "greedy gc prio", IO_limit) );
	exp.push_back( Experiment::overprovisioning_experiment(lazy_eq_prio,		space_min, space_max, space_inc, exp_folder + 		"lazy_eq_prio/", "lazy eq prio", IO_limit) );
	exp.push_back( Experiment::overprovisioning_experiment(lazy_gc_priorty,	space_min, space_max, space_inc, exp_folder + 		"lazy_gc_priorty/", "lazy gc priorty", IO_limit) );
	...
	Experiment::graph(sx, sy,   "Throughput", 				"throughput", 				24, exp);
	Experiment::graph(sx, sy,   "Write Throughput", 			"throughput_write", 		25, exp);
	Experiment::graph(sx, sy,   "Read Throughput", 			"throughput_read", 			26, exp);
	Experiment::graph(sx, sy,   "Num Erases", 				"num_erases", 				8, 	exp);
	Experiment::graph(sx, sy,   "Num Migrations", 			"num_migrations", 			3, 	exp);

	Experiment::graph(sx, sy,   "Write wait, mean", 			"Write wait, mean", 		9, 	exp);
	Experiment::graph(sx, sy,   "Write wait, max", 			"Write wait, max", 			14, exp);
	Experiment::graph(sx, sy,   "Write wait, std", 			"Write wait, std", 			15, exp);
	...
	Experiment::cross_experiment_waittime_histogram(sx, sy/2, "waittime_histogram 88", exp, 80, 1, 4);
	Experiment::cross_experiment_waittime_histogram(sx, sy/2, "waittime_histogram 95", exp, 70, 1, 4);
