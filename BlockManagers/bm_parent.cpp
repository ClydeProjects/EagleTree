...
void Block_manager_parent::Wear_Level(Event const& event) {
	...
	if (ENABLE_WEAR_LEVELING && blocks_to_wl.size() == 0 && blocks_being_wl.size() == 0 && max_age > get_min_age() + 50) {
...
vector<deque<Event*> > Block_manager_parent::migrate(Event* gc_event) {
	...
	for (uint i = 0; i < BLOCK_SIZE; i++) {
		if (victim->getPages()[i].get_state() == VALID) {
			...
			Address copy_back_target = !is_wear_leveling_op && copy_back_allowed_on(logical_address) ? reserve_page_on(addr.package, addr.die, gc_event->get_current_time()) : Address();
