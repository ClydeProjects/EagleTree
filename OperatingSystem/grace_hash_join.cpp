...
Event* Grace_Hash_Join::execute_build_phase() {
	...
	// If we just finished reading a full relation, start next task (either start reading next relation, or finish off the BUILD phase)
		...
		if (writes_in_progress > 0) return NULL; // Wait for the last writes to finish before continuing
...
Event* Grace_Hash_Join::execute_probe_phase() {
	...
	bool finished_with_current_bucket = (small_bucket_cursor > small_bucket_end && large_bucket_cursor > large_bucket_end && trim_cursor > large_bucket_end);
	if (first_run || finished_with_current_bucket) {
		...
		if (victim_buffer >= num_partitions) {
			phase = DONE;
			return NULL;
		}

		// Set cursors corresponding to the chosen buffer
		if (PRINT_LEVEL >= 1) printf("Grace Hash Join, probe phase: Probing buffer #%d\n", victim_buffer);
